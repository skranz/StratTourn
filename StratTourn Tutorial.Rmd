...
The columns `obs.i` and `obs.j` show the observations at the beginning of a period, here just the previous actions of player i hersefl (obs.i) and the other player (obs.j). The columns err.D.i and err.D.j help to see whether there was an observation error and a cooperation (C) has been observed as defection (D).
...
```{r eval=TRUE, cache=TRUE}
  ...
  setwd("D:/libraries/StratTourn/studies") # adapt the folder
...
There are different ways how one can evaluate the performance of the submitted strategies and give scores to different strategies. I want to present the basic idea of some approaches. The exact scoring rules will be explained in the task descriptions during the course.


### 5.1 Average performance against all strategies

In his original tournament Axelrod let each strategy play against each other several repeated games and the score of each strategy was its average payoff. 

### 5.2 Evolutionary dynamic and performance in weighted population

Imagine we have a society that lasts for several generations and has a big population of individuals. Each individual has a strategy that it plays in the repeated game. So we can think of each strategy having a share in the total population.   

  1. **Average performance** How well does your strategy perform on average against all submitted strategies, including the payoff against itself. (This was Axelrod's original criterion)
  
  2. **Evolutionary setting** We consider an evolutionary dynamic in which successful strategies grow and unsuccessful strategies shrink. The success of a strategy still depends on its payoff against itself and other submitted strategies, but it becomes especially important to perform well against successful strategies.

  3. **Efficient and stable social norm** We 
...
```{r cache=TRUE, tidy=FALSE, fig.height=4, fig.width=4}
  library(StratTourn)
...
```{r cache=TRUE, fig.height=4, fig.width=9}
  sim = NULL # reset sim
