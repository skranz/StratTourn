...
#### How states transfer information between periods:
```
The value of the state that is passed to your function in period t is the value of the state that your function has returned in period t-1. (The value of a state in period 1 is the value you specify as argument in the function definition).
```
...
```{r eval=TRUE, cache=!TRUE}
  run.rep.game(delta=0.9, game=game, 
               strat = nlist(strange.defector,tit.for.tat),T.min=25)
```
...
```{r eval=TRUE, cache=!TRUE}
  run.rep.game(delta=0.9, game=game, 
               strat = nlist(ashamed.defector,random.action),T.min=25)
```
```{r include=FALSE}
...
The first matrix has for every strategy a row and column and shows the average payoff of the *row strategy*  when the row strategy plays against the column strategy. (In a symmetric game like the PD game, we will just display the average payoffs of both player 1 and player 2). For example, when strategy always.defect played against always.coop, then always.defect got on average a payoff of  `r round(tourn$mat[3,4],3)` and always.coop got on average a payoff of `r round(tourn$mat[4,3],3)`. Against itself always.coop got an average payoff of `r round(tourn$mat[4,4],3)`.
