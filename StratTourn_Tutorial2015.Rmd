...
**with help by Martin Kies (martin.kies@uni-ulm.de) and Daniel Blochinger (daniel.blochinger@uni-ulm.de)**
...
install_github(repo="skranz/shinyEvents")
install_github(repo="skranz/shinyPart")
install_github(repo="skranz/codeUtils")
install_github(repo="skranz/WhitelistEval")
...
We do this search by programming strategies in R and let them play against each other in a tournament (more on that below) and then rank strategies against each other. In different tasks we will use different criteria to rank the strategies into more or less succesful strategies. The criteria will be described for each task in the seminar.
...
## 5. Playing against other strategies in the web

In many tasks you will be able to check the performance of your strategy against a set of strategies that have been previously submitted. You can do this on the following website:

[econfin.de:3838/battle](econfin.de:3838/battle)

The password is given to you in the seminar.
...
### Note: This section was created for an older instance of the seminar, were we had an explicit notion of a strategy's stability. The test functions may nevertheless still be helpful
