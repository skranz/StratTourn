...
# Set size from init.size
init.size = rep(1,NROW(used.strats))
names(init.size) = used.strats

vec = NULL
try(vec <- eval(parse(text=paste0("c(",init.size.string,")"))))

cols = intersect(names(init.size), names(vec))
init.size[cols] = vec[cols]
init.size = init.size / sum(init.size)
