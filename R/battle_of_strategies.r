...
evolve = function(initial=rep(1/NROW(mat),NROW(mat)),mat,rounds = 100,alpha=0.1, min.share = 0.001) {
  shares = matrix(NA,rounds,length(initial))
  shares[1,] = initial
  colnames(shares) = rownames(mat)
  for (r in 2:rounds) {
    shares[r,] = evolve.one.round(shares=shares[r-1,],mat=mat,alpha=alpha, min.share=min.share) 
  }
  return(shares)
}
...
evolve.one.round = function(shares,shares.j=shares,mat,alpha=0.1, min.share=0.001) {
  restore.point("evolve.one.round")
  fit = mat %*% shares.j
  shares = pmax(min.share,shares + alpha*(fit-sum(shares*fit))*shares)
  shares/sum(shares)
}
